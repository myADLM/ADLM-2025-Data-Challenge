...
def init_embedding_model(aws_region: str = "us-east-1"):
    """Initialize Bedrock embedding model."""
    print(f"Loading embedding model: Bedrock Titan...")
    embed_model = BedrockEmbedding(
        model_name="amazon.titan-embed-text-v2:0",
        region_name=aws_region,
        embed_batch_size=10
    )
    print(f"  Using Bedrock Titan embeddings (1024 dimensions, batch_size=10, max_tokens=8192)")
    return embed_model
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Index markdown documents with Bedrock embeddings")
    ...
    embed_model = init_embedding_model(args.aws_region)
